...
function initial()
{
    Liste = new Array((groesse*2)+1);
    for (var i = 0; i < (groesse*2)+1; i++) {
        Liste[i] = new Array((groesse*2)+1);
    }
    
    //Get svg element
    for(i=0;i<(groesse*2)+1;i++)
        for (j = 0; j < (groesse*2)+1;j++)
        {
            if(!((j+i<groesse)| (((groesse*2)-j)+((groesse*2)-i) <groesse)))
            {
            var svg = document.getElementsByTagName('svg')[0];
            var x = pixel + (pixel * i)+(j*pixel/2);

            var y = pixel + (pixel * j *0.9);
            var r = pixel/2;
